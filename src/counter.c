...
R_xlen_t cli_counter_Length(SEXP x) {
  SEXP data1 = R_altrep_data1(x);
  return INTEGER(data1)[0];
}

void* cli_counter_DataPtr(SEXP x, Rboolean writeable) {
  SEXP data2 = R_altrep_data2(x);
  return (void*) INTEGER(data2);
}

int cli_counter_Elt(SEXP x, R_xlen_t i) {
  SEXP data2 = R_altrep_data2(x);
...
void cli_init_altrep(DllInfo *dll) {
  ...
  R_set_altvec_Dataptr_method(cov_counter_t, cli_counter_DataPtr);
  R_set_altinteger_Elt_method(cov_counter_t, cli_counter_Elt);
}
